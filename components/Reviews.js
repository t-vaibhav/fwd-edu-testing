...
export const Reviews = () => {
    ...
    const studentReviewsCard = studentReviews.map((review, index) => {
        ...
        return (
            <div className='sm:flex items-center my-20 max-w-screen-[40vh] px-2' key={index}>
    ...
    const parentReviewsCard = parentReviews.map((review, index) => {
        return (
            <>
                <div className='sm:flex items-center my-20 max-w-screen-[40vh] px-2' key={index}>
                    <div className='sm:mr-12 flex justify-center'>
                        <Image src={`/images/reviews/${review.img}`} className='rounded-md sm:h-[120px] sm:max-w-[110px] h-[200px] max-w-[175px] mb-4 sm:mb-0' width={600} height={600} alt={review.parent} />
                    </div>
                    <div>
                        <p className='text-gray-800 '>{review.txt}</p>
